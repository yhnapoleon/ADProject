...
class FoodClassifier:
	...
	def __init__(self, asian_labels_path: str = "asian_food_labels.txt", asia_score_weight: float = 1.1) -> None:
		...
		self.western_processor = AutoImageProcessor.from_pretrained(self.western_model_name)
		self.western_model = AutoModelForImageClassification.from_pretrained(self.western_model_name).to(self.device)
		self.western_model.eval()

		# 亚洲模型改为 CLIP：以文本候选标签进行匹配
		self.asian_processor = CLIPProcessor.from_pretrained(self.asian_model_name)
		self.asian_model = CLIPModel.from_pretrained(self.asian_model_name).to(self.device)
		...
		if os.path.isabs(asian_labels_path):
			...
			candidate_paths.append(os.path.join(os.path.dirname(__file__), asian_labels_path))
			candidate_paths.append(os.path.join(os.getcwd(), asian_labels_path))

		label_file = next((p for p in candidate_paths if os.path.exists(p)), None)
		...
		with open(label_file, "r", encoding="utf-8") as f:
			self.asian_labels = [line.strip() for line in f.readlines() if line.strip()]
	...
	def predict(self, image_path: str) -> Dict[str, Any]:
		...
		a_inputs = self.asian_processor(text=self.asian_labels, images=image, return_tensors="pt", padding=True)
		...
		print(f"[FoodClassifier] Compare -> west: {w_top_score:.6f}, asia_raw: {a_top_score:.6f}, asia_weighted({self.asia_score_weight:.2f}x): {asia_weighted:.6f}")
