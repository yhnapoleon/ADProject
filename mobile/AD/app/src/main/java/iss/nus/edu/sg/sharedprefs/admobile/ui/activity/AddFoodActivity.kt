...
class AddFoodActivity : AppCompatActivity() {
    ...
    private val pickImageLauncher = registerForActivityResult(ActivityResultContracts.GetContent()) { uri: Uri? ->
        uri?.let { showImage(it); uploadAndAnalyze(it) }
    }

    private val takePictureLauncher = registerForActivityResult(ActivityResultContracts.TakePicturePreview()) { bitmap: Bitmap? ->
        bitmap?.let { showImageBitmap(it); uploadAndAnalyze(saveBitmapToTempFile(it)) }
    }
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Toast.makeText(this@AddFoodActivity, "Calculation failed: ${response.code()}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body()?.success == true) {
                    ...
                    Toast.makeText(this@AddFoodActivity, "Save failed: ${response.code()}", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful && response.body() != null) {
                    ...
                    Toast.makeText(this@AddFoodActivity, "Detected: $label", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
                Log.e("ECO_DEBUG", "Analysis Error: ${e.message}")
            } finally {
