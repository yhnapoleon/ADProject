	...
	public Task<DocumentTypeClassificationResult> ClassifyAsync(string ocrText, CancellationToken ct = default)
	{
		if (string.IsNullOrWhiteSpace(ocrText))
		{
			return Task.FromResult(new DocumentTypeClassificationResult
			{
				...
				ConfidenceScore = 0,
				ErrorMessage = "OCR text is empty; cannot identify document type."
			});
		}
		...
		else if (flightScore >= 2)
		{
			...
			errorMessage = "Uploaded image is a flight ticket/boarding pass, not a utility bill. Please upload Singapore electricity, water or gas bill.";
		}
		else if (receiptScore >= 3)
		{
			documentType = DocumentType.Receipt;
			errorMessage = "Uploaded image is a receipt, not a utility bill. Please upload Singapore electricity, water or gas bill.";
		}
		else if (invoiceScore >= 2)
		{
			documentType = DocumentType.Invoice;
			errorMessage = "Uploaded image is an invoice, not a utility bill. Please upload Singapore electricity, water or gas bill.";
		}
		else
		{
			documentType = DocumentType.Unknown;
			errorMessage = "Cannot identify uploaded image type. Please ensure you upload Singapore electricity, water or gas bill.";
		}
